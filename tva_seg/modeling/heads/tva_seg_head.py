...
from ..transformer.tva_seg_predictor import TvaSegPredictor


@SEM_SEG_HEADS_REGISTRY.register()
class TvaSegHead(nn.Module):
    ...
    def from_config(cls, cfg, input_shape: Dict[str, ShapeSpec]):
        return {
            ...
            "transformer_predictor": TvaSegPredictor(
                cfg,
            ),
            "clip_pretrained": cfg.MODEL.SEM_SEG_HEAD.CLIP_PRETRAINED,
        }

    def forward(self, features,clip_features, dino_featss, all_features, vfm_feats, filtered_ids=None, prompt=None, gt_cls=None):
        """
        Arguments:
            img_feats: (B, C, HW)
        ...
        if 'eva' in self.clip_name.lower():
            ...
            img_feat = rearrange(features[:, 1:, :], "b (h w) c->b c h w", h=self.feature_resolution[0], w=self.feature_resolution[1])
        return self.predictor(img_feat,clip_features, all_feat, dino_featss, all_features, vfm_feats, filtered_ids, prompt, gt_cls)