...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, appearance_guidance_dim, num_heads, head_dim=None, window_size=7, qkv_bias=True, attn_drop=0., proj_drop=0.):
        ...
        self.q = nn.Linear(dim + 2*appearance_guidance_dim, attn_dim, bias=qkv_bias)
        self.k = nn.Linear(dim + 2*appearance_guidance_dim, attn_dim, bias=qkv_bias)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, appearance_guidance,dino_guidance):
        ...
        if appearance_guidance is not None:
            ...
            x = torch.cat([x, appearance_guidance, dino_guidance], dim=-1)
...
class SwinTransformerBlockWrapper(nn.Module):
    def __init__(self, dim, appearance_guidance_dim, input_resolution, nheads=4, window_size=5, pad_len=0):
        ...
        self.guidance_norm1 = nn.LayerNorm(appearance_guidance_dim) if appearance_guidance_dim > 0 else None
    ...
    def forward(self, x, appearance_guidance, dino_guidance):
        ...
        if appearance_guidance is not None:
            ...
            dino_guidance = self.guidance_norm1(repeat(dino_guidance, 'B C H W -> (B T) (H W) C', T=T))
...
class Up(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, clip_guidance_channels, dino_guidance_channels):
        ...
        self.conv = DoubleConv(in_channels+2*dino_guidance_channels, out_channels)
        # Define separate convolutional layers for guidance and dino_guidance
        self.cconv_guidance = nn.Conv2d(in_channels - clip_guidance_channels, clip_guidance_channels, kernel_size=1, stride=1,
                                        padding=0)
        self.sconv_guidance = nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0)
        self.cconv_dino = nn.Conv2d(in_channels, dino_guidance_channels, kernel_size=1, stride=1,
                                    padding=0)
        self.sconv_dino = nn.Conv2d(1, 1, kernel_size=1, stride=1, padding=0)

    def forward(self, x,clip_guidance, dino_guidance, clip_corr):
        x = self.up(x)
        if clip_guidance is not None:
            T = x.size(0) // clip_guidance.size(0)
            attention_clip = x.view(clip_guidance.size(0), T, x.size(1), clip_guidance.size(2), clip_guidance.size(3)).mean(dim=1)
            channel_attention_clip = attention_clip.mean(dim=[2, 3], keepdim=True)
            spatial_attention_clip = attention_clip.mean(dim=1, keepdim=True)
            channel_attention_clip = self.cconv_guidance(channel_attention_clip)
            spatial_attention_clip = self.sconv_guidance(spatial_attention_clip)
            clip_guidance = channel_attention_clip * clip_guidance + spatial_attention_clip * clip_guidance + clip_guidance
        ...
        if dino_guidance is not None:
            ...
            attention_dino = x.view(dino_guidance.size(0), T, x.size(1), dino_guidance.size(2), dino_guidance.size(3)).mean(dim=1)
            channel_attention_dino = attention_dino.mean(dim=[2, 3], keepdim=True)
            spatial_attention_dino = attention_dino.mean(dim=1, keepdim=True)
            channel_attention_dino = self.cconv_dino(channel_attention_dino)
            spatial_attention_dino = self.sconv_dino(spatial_attention_dino)
            dino_guidance = channel_attention_dino * dino_guidance + spatial_attention_dino * dino_guidance + dino_guidance
            dino_guidance = repeat(dino_guidance, "B C H W -> (B T) C H W", T=T)
            x = torch.cat([x, dino_guidance], dim=1) 
        x = torch.cat([x, clip_corr], dim=1) 
        return self.conv(x)
...
class Aggregator(nn.Module):
    def __init__(self, 
        ...
        self.guidance_projection1 = nn.Sequential(
        ...
        self.clip_guidance_projection = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(d, dp, kernel_size=3, stride=1, padding=1),
                nn.ReLU(),
            ) for d, dp in zip(decoder_guidance_dims, decoder_guidance_proj_dims)
    ...
    def conv_decoder(self, x,clip_features, guidance, corr_guidance):
        B = x.shape[0]
        corr_embed = rearrange(x, 'B C T H W -> (B T) C H W')
        corr_embed = self.decoder1(corr_embed,clip_features[0], guidance[0], corr_guidance[0])
        corr_embed = self.decoder2(corr_embed,clip_features[1], guidance[1], corr_guidance[1])
        corr_embed = self.head(corr_embed)
        corr_embed = rearrange(corr_embed, '(B T) () H W -> B T H W', B=B)
        return corr_embed

    def category_filtering_module(self, cost_maps, text_feats, filtered_ids=None, lambda_val=0.1, p=32, k=3):
        D = cost_maps.mean(dim=1, keepdim=False)  # Average to get (B, T, H, W)
        D_flat = D.view(D.size(0), D.size(1), -1)  # Flatten to (B, T, H*W)
        top_k_values, top_k_indices = D_flat.topk(k, dim=1, largest=True, sorted=True)  # Select the top-k classes
        B, T, H, W = D.size(0), D.size(1), D.size(2), D.size(3)
        matched = (top_k_indices[:, :, None, :] == torch.arange(T, device=cost_maps.device).view(1, 1, T, 1))  # [B, k, T, H*W]
        matched_sum = matched.sum(dim=-1)  # Sum over the H*W dimensions to get the score for each class: (B, k, T)
        R = (lambda_val ** torch.arange(k, device=cost_maps.device)[None, :, None]) * matched_sum  # (B, k, T)
        # Sort scores along the class dimension and select the top-P classes
        R_sum = R.sum(dim=1)  # (B, T)
        R_sorted, R_indices = torch.sort(R_sum, dim=1, descending=True)  # Sort the scores and obtain the indices
        top_P_indices = R_indices[:, :p]  # Sort the scores and get the indices
        if filtered_ids is not None:
            # Flatten filtered_ids to a 1D array
            filtered_ids = filtered_ids.flatten()
            # Set the total target number p (e.g., 32)
            total_p = p  # total numbers
            # If the number of filtered_ids is less than the target, add extra classes
            num_gt = filtered_ids.numel()
            if num_gt < total_p:
                # Select additional classes from top_P_indices by first flattening it
                available = top_P_indices.flatten()
                # Exclude classes that are already in filtered_ids
                available = available[~torch.isin(available, filtered_ids)]
                # Select the first (total_p - num_gt) additional classes
                additional = available[: (total_p - num_gt)]
                final_top_P_indices = torch.cat((filtered_ids, additional))
            else:
                # If the number of GT classes is greater than or equal to the target, select only the first total_p GT classes
                final_top_P_indices = filtered_ids[:total_p]
            # Reshape to [1, total_p]
            final_top_P_indices = final_top_P_indices.unsqueeze(0)
            top_P_indices = final_top_P_indices
        # Adjust the index dimensions to match the gather operation
        indices_expanded = top_P_indices[:, None, :, None, None].expand(-1, cost_maps.size(1), -1, cost_maps.size(3), cost_maps.size(4))
        indices_text = top_P_indices[:, :, None,None].expand(-1, -1, text_feats.size(2),text_feats.size(3))
        # Slice along the class dimension (T dimension)
        Sr = torch.gather(cost_maps, dim=2, index=indices_expanded)
        text= torch.gather(text_feats, dim=1, index=indices_text)
        return Sr, text, top_P_indices
    def forward(self, img_feats,clip_features, text_feats, dino_featss, vfm_feats, filtered_ids=None):
        """
        Arguments:
            img_feats: (B, C, H, W)
            text_feats: (B, T, P, C)
        ...
        """
        classes = None
        _,T,_,_=text_feats.shape
        corr = self.correlation(img_feats, text_feats)
        corr, text_feats, selected_indices = self.category_filtering_module(corr, text_feats, filtered_ids)
        ...
        clip_guidance_features = [None, None]
        dino_guidance = None
        if self.guidance_projection is not None:
            projected_guidance = self.guidance_projection(clip_features[0])
        if self.guidance_projection1 is not None:
            dino_guidance = self.guidance_projection1(dino_featss[0])
        if self.decoder_guidance_projection is not None:
            projected_decoder_guidance = [proj(g) for proj, g in zip(self.decoder_guidance_projection, dino_featss[1:])]
        if self.clip_guidance_projection is not None:
            clip_guidance_features = [proj(g) for proj, g in zip(self.clip_guidance_projection, clip_features[1:])]
        ...
        for layer in self.layers:
            corr_embed = layer(corr_embed, projected_guidance,dino_guidance,projected_text_guidance)
        logit = self.conv_decoder(corr_embed, clip_guidance_features, projected_decoder_guidance,projected_corr_decoder_guidance)
        selected_indices_expanded = selected_indices.unsqueeze(-1).unsqueeze(-1).expand(B, P, logit.size(2), logit.size(3))
        logit_full = torch.full((B, T, logit.size(2), logit.size(3)), -100., device=logit.device)
        logit_full.scatter_(dim=1, index=selected_indices_expanded, src=logit)
        logit=logit_full
